...
function FindTheBadge {

  BadgeCompare1=$1;
  BadgeCompare2=$2;
  BadgeCompare3=$3;
  
  for (( j=0; j<${#BadgeCompare1}; j++ )); do
    if [[ "$BadgeCompare2" == *"${BadgeCompare1:$j:1}"* ]]; then
      if [[ "$BadgeCompare3" == *"${BadgeCompare1:$j:1}"* ]]; then
        translateToPriority "${BadgeCompare1:$j:1}" #Single matching character
        echo "Priority of matching item ${contentsA:$j:1} is $addvalue"
  ...
  BagBreakdown
  echo "Total Bags: ${#Bag[@]}"
  echo "Total Teams: $((${#Bag[@]}/3))"
  for (( i=0; i<$((${#Bag[@]}/3)); i++));  do
    echo "Team $i - Bag Numbers $(($threeMultiples)) $(($threeMultiples+1)) $(($threeMultiples+2))"
      threeMultiples=$(($i*3))
      FindTheBadge "${Bag[$(($threeMultiples))]}" "${Bag[$(($threeMultiples+1))]}" "${Bag[$(($threeMultiples+2))]}"
  done
echo $TotalPriority
